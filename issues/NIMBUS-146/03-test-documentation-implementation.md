...
**Known blocker for TC-1, TC-5, TC-6:** these require NIMBUS-129's `POST /orderapi/orders`
...
token), TC-3 (missing customer number), TC-4 (malformed list entry), and TC-7 (HTTPS) can be
executed as soon as Task 02's Logic App itself is deployed, independent of Medusa's deployment
state.
...
### TC-4: Matched token whose list entry has no `::` delimiter — not allowed (edge case)
- **Given:** Seed a token-list entry `Value = "no-delimiter-token"` — the token is present but the
  customer number was never encoded. This is distinct from TC-3: the entry is not empty after the
  delimiter, it has no delimiter at all, so a naive `last(split(...))` extraction would resolve the
  customer number to the token itself.
- **When:** A tester sends a structurally valid order to:
  \`\`\`
  ...
  { "externalOrderNumber": "MANUAL-TEST-4", "orderDate": "2026-09-02", "currencyCode": "DKK",
    "lines": [{"lineNumber":1,"itemNumber":"ITEM-1","eanNo":"1234567890123","description":"Test item","quantity":1,"unitPrice":10}] }
  \`\`\`
- **Then:** The response is `401` with body exactly `{"error": "Not allowed"}`. In the run history,
  confirm `Filter_Matching_Token` *did* match the entry, that `Compose_Customer_Number`'s output is
  the empty string (**not** `no-delimiter-token`), and that `Forward_Order_To_Medusa` was skipped —
  the live counterpart of Task 02's TC-3 regression guard.

### TC-5: Valid token, structurally invalid body — downstream 400 passes through (error condition)
...
- **Then:** The response is `400` (Medusa's `validateAndTransformBody` rejection, per NIMBUS-144's
  ...
  counterpart of Task 02's TC-5, which traces the same behavior at the expression level). Also
  confirm the run history shows the forwarded order body but **not** the Medusa API key.

### TC-6: Valid token, unknown customer number — downstream 404 passes through (wiring/integration)
...
### TC-7: HTTPS enforcement (documentation-only verification)
...
- **When:** Cross-referencing against `test-payloads.md`'s TC-1 through TC-7.
- **Then:** Each behavior maps to at least one documented test case (TC-1/TC-5/TC-6 cover
  receive+validate+forward+downstream pass-through, TC-2 covers an invalid token, TC-3 and TC-4
  cover the two malformed-entry shapes that must not resolve a customer number, and TC-7 covers
  HTTPS).

### TC-2: Rejection test cases never expose authorization data (edge case, consistency)
- **Given:** TC-2, TC-3, and TC-4's documented expected responses.
...
- **Then:** `test-payloads.md`'s header explicitly states which test cases (TC-1, TC-5, TC-6) are
  blocked on that dependency and which (TC-2, TC-3, TC-4, TC-7) are not — so a future reader
  doesn't assume all seven are immediately executable.
